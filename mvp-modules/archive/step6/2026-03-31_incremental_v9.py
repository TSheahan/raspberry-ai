"""Incremental v9 — Clean production script.

Base: voice_pipeline_step6_first_attempt.py
Applied: wanted changes W1-W9 from Section 1b of the iteration guide.
Discarded: all troubleshooting jitter J1-J10.
"""
...
from openwakeword.model import Model  # W5: no bare 'import openwakeword'
...
class OpenWakeWordProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: str):  # W6
...
class UtteranceCapturer(FrameProcessor):
    ...
    def start_capture(self):
        self.capturing = True
        self.utterance_buffer = np.array([], dtype=np.int16)  # W3
...
class DeepgramSTTProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: str):  # W6 — critical
        await super().process_frame(frame, direction)
        await self.push_frame(frame, direction)

    # W4: post-pipeline transcription instead of inline
...
async def main():
    ...
    runner = PipelineRunner()  # W7: default handle_sigint=True
    task = PipelineTask(pipeline)

    print("Step 6 v9 — Listening...")
    print("   Say 'hey Jarvis', speak a short sentence, then Ctrl+C to test shutdown.")
    print("   Press Ctrl+C to test shutdown.\n")

    await runner.run(task)  # W8: plain await, no try/except wrapper; W9: no manual cleanup

    print("\nRunning final transcription...")
    stt_processor.transcribe_sync()

    print("Step 6 v9 finished cleanly -- process should exit now.")
