...
main_loop = None
pipeline_task = None   # ← will hold the PipelineTask reference
transport = None
stt_processor = None
...
async def shutdown_task():
    ...
    # CRITICAL: explicitly cancel the pipeline so runner.run() returns
    if pipeline_task:
        print("🔪 [TRIPWIRE] Cancelling PipelineTask")
        pipeline_task.cancel()

    print("✅ [TRIPWIRE] shutdown_task completed — expecting pipeline to exit now")

class OpenWakeWordProcessor(FrameProcessor):
    # (unchanged from v3 — kept for brevity; same cancellation-aware code)
...
async def main():
    ...
    pipeline_task = PipelineTask(pipeline)   # ← global reference for shutdown

    print("🎤 Step 6 — Pipecat v4 with explicit cancellation + tripwires")
    ...
    try:
        ...
        print("\n🛑 [TRIPWIRE] Pipeline task cancelled cleanly")
    except Exception as e:
        print(f"\n⚠️  Unexpected error: {e}")
    finally:
        print("\n🧹 [TRIPWIRE] main() finally block reached — final cleanup")
        if stt_processor:
            stt_processor._transcribe_sync()
        if transport:
            await transport.cleanup()
