...
dg_client = None
audio_stream = None
loop = None  # will hold the main asyncio loop

def audio_callback(in_data, frame_count, time_info, status):
    """PyAudio callback — runs in a separate thread. No asyncio calls here."""
    global utterance_buffer, capturing, last_detection_time

    audio_chunk = np.frombuffer(in_data, dtype=np.int16)

    # Wake-word detection (thread-safe time)
    ...
    for wakeword, score in predictions.items():
        if (wakeword == "hey_jarvis" and 
            ...
            capturing = True
            utterance_buffer = np.array([], dtype=np.int16)   # reset
...
def shutdown():
    """Clean shutdown — called from main thread."""
    ...
    print("✅ Diagnostic v2 finished cleanly — process should exit now.")
    sys.exit(0)

def handle_sigint(signum, frame):
    """Signal handler — schedules shutdown on the main asyncio loop."""
    if loop:
        loop.call_soon_threadsafe(lambda: asyncio.create_task(asyncio.to_thread(shutdown)))
...
async def main():
    ...
    print("🎤 Minimal diagnostic v2 (no Pipecat): Listening for 'hey jarvis' …")
    print("   After wake word, speak a short sentence then pause.")
    print("   Press Ctrl+C when finished speaking.\n")

    # Keep the loop alive
    try:
        await asyncio.Event().wait()  # never set — we shut down via signal
