...
main_loop = None
pipeline_task = None
runner_task = None      # ← NEW: wrapper around runner.run()
transport = None
stt_processor = None
...
async def shutdown_task():
    ...
    global stt_processor, transport, pipeline_task, runner_task
    ...
    if runner_task:
        ...
        try:
            await runner_task
            print("✅ [TRIPWIRE] runner_task completed")
        except asyncio.CancelledError:
            print("✅ [TRIPWIRE] runner_task was cancelled (expected)")

    print("✅ [TRIPWIRE] shutdown_task completed — main() should now exit")
...
async def main():
    global main_loop, pipeline_task, runner_task, transport, stt_processor
    ...
    print("🎤 Step 6 — Pipecat v6 (runner_task + full cancellation)")
    print("   After wake word, speak a short sentence then pause.")
    print("   Press Ctrl+C when finished speaking.\n")

    runner_task = asyncio.create_task(runner.run(pipeline_task))   # ← THIS IS THE KEY

    try:
        print("▶️  [TRIPWIRE] Waiting for runner_task")
        await runner_task
        print("▶️  [TRIPWIRE] runner_task returned normally")
    except asyncio.CancelledError:
        print("🛑 [TRIPWIRE] runner_task was cancelled (caught in main)")
    except Exception as e:
        print(f"⚠️  [TRIPWIRE] Unexpected error in runner_task: {e}")
    finally:
