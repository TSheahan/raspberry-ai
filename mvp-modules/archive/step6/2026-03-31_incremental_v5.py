"""Incremental v5 — Add DeepgramSTTProcessor + post-pipeline transcription.

Pipeline: LocalAudioTransport.input() -> OpenWakeWordProcessor -> UtteranceCapturer -> DeepgramSTTProcessor
After runner.run() completes, run final transcription synchronously.
This matches v8's full feature set. Tests whether the complete pipeline exits cleanly.
...
class UtteranceCapturer(FrameProcessor):
    """Identical to v4's UtteranceCapturer."""
...
class OpenWakeWordProcessor(FrameProcessor):
    """Identical to v4's OpenWakeWordProcessor."""
...
class DeepgramSTTProcessor(FrameProcessor):
    """Identical to v8's DeepgramSTTProcessor — sits in pipeline but
    transcription is called post-pipeline (not during frame processing)."""
...
async def main():
    ...
    print("Incremental v5 (Pipecat + OpenWakeWord + UtteranceCapturer + DeepgramSTT): Listening...")
    ...
    print("Incremental v5 finished cleanly -- process should exit now.")
