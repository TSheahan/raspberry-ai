"""Step 7 v3 — Wake word + VAD + STT + agentic layer (claude -p).
...
v3 changes from v2:
  - Full OWW model reset on gated→ungated transition. model.reset() only clears the
    prediction_buffer deque; the preprocessor's raw_data_buffer, melspectrogram_buffer,
    feature_buffer, and accumulated_samples survive, causing stale features to produce
    false wake detections the moment gating lifts. Now we clear all of these.
  - Track _was_gated to detect the transition edge.
  - Debounce timer updated on reset to prevent immediate re-trigger.

Provenance: builds on voice_pipeline_step7_v2.py.
"""
...
class OpenWakeWordProcessor(FrameProcessor):
    ...
    def _reset_model_full(self):
        ...
        """
        # Prediction buffer (what model.reset() does)
        self.model.prediction_buffer = dict.__class__(
...
async def main():
    ...
    print("Step 7 v3 -- Wake word + VAD + STT + Claude")
