"""Step 7 v10a — Stream suspension during processing (no state break-out).

v10a changes from v10:
  - Stream pause/resume: when the cognitive loop starts (processing phase),
    the PyAudio input stream is stopped to prevent buffer overflow during
    CPU-heavy Claude subprocess.  On completion, Silero hidden states are
    reset BEFORE the stream is resumed (ordering matters).
  - All stream ops use async helpers scheduled as tasks — stop_stream() is
    never called synchronously from within a Pipecat frame-processing callback
    (PortAudio deadlock / USB fault on Pi).
  - Selective VAD conditional from v11: only StartFrame + audio-during-capture
    reach the VAD controller.  CancelFrame/EndFrame never reach it (crash fix).
  - Retains v10's direct processor cross-references (no PipelineState object).

Provenance: builds on voice_pipeline_step7_v10.py + stream gating from v11.
"""
...
# ---------------------------------------------------------------------------
# Stream suspension helpers
# ---------------------------------------------------------------------------

async def _pause_stream(transport_ref):
    """Stop PyAudio input stream to prevent buffer overflow during processing."""
    if transport_ref and hasattr(transport_ref, '_in_stream') and transport_ref._in_stream:
        try:
            print("  [stream] audio stream to pause..")
            transport_ref._in_stream.stop_stream()
            print("  [stream] audio stream paused")
        except Exception as e:
            print(f"  [stream] pause failed: {e}")
    else:
        print("  [stream] pause skip — no active _in_stream")


async def _resume_stream(transport_ref):
    """Resume PyAudio input stream."""
    if transport_ref and hasattr(transport_ref, '_in_stream') and transport_ref._in_stream:
        try:
            transport_ref._in_stream.start_stream()
            print("  [stream] audio stream resumed")
        except Exception as e:
            print(f"  [stream] resume failed: {e}")
    else:
        print("  [stream] resume skip — no active _in_stream")
...
class UtteranceCapturer(FrameProcessor):
    ...
    def __init__(self):
        ...
        self._vad_weak = None
        self._transport_ref = None  # set in main() after transport exists
    ...
    def start_capture(self):
        ...
        # Reset VAD the instant wake word is detected
        if self._vad_weak:
            vad = self._vad_weak()
            if vad is not None:
        ...
        if isinstance(frame, VADUserStoppedSpeakingFrame) and self.capturing:
            ...
            if GATE_VAD_ALL:
                ...
                if GATE_VAD_CREATE_TASK:
                    ...
                    # Pause stream BEFORE launching cognitive loop
                    asyncio.create_task(self._pause_then_process(audio))

        await self.push_frame(frame, direction)

    async def _pause_then_process(self, audio: np.ndarray):
        """Pause the audio stream, then run the cognitive loop."""
        await _pause_stream(self._transport_ref)
        await self._cognitive_loop(audio)
    ...
    async def _cognitive_loop(self, audio: np.ndarray):
        ...
        try:
            ...
            self.processing = False
            # Reset VAD BEFORE resuming stream — ordering matters
            if self._vad_weak:
                vad = self._vad_weak()
                if vad is not None:
                    await vad.reset_vad()
            await _resume_stream(self._transport_ref)
            print("\nListening for wake word...")
...
async def main():
    ...
    # Cross-references (no PipelineState — direct wiring)
    ...
    # Wire transport ref for stream suspension
    capturer._transport_ref = input_transport
    ...
    print("Step 7 v10a — stream suspension during processing (no state break-out)")
