...
Provenance: builds on voice_pipeline_step7_v01a.py.

v01b change: Replace VADProcessor with GatedVADProcessor.
Silero ONNX inference is now gated — only runs during capturer.capturing.
Outside that window, audio frames are forwarded without touching ONNX.
Both OWW and Silero are now gated to non-overlapping phases, eliminating
concurrent CPU loads during the cognitive loop.

Includes speech_started / speech_stopped prints as soon as the handlers
exist — no reason to defer visibility to a later increment.
"""
...
class GatedVADProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        # StartFrame always reaches controller (sets sample rate for Silero).
        # All other frames only reach controller during capture window.
...
async def main():
    ...
    # capturer must be constructed before GatedVADProcessor (holds reference to capturer.capturing)
    ...
    pipeline = Pipeline([
        transport.input(),
        vad_processor,        # GatedVAD first — emits start/stop speaking frames; gated to capturing only
    ...
    print("Step 7 v01b -- Wake word + GatedVAD + STT + Claude")
