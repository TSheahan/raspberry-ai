...
class GatedVADProcessor(FrameProcessor):
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, (AudioRawFrame, InputAudioRawFrame)):
            # Audio frames are the only ones we ever gate
            ...
            # USER NOTE FOR AGENT - while troubleshooting deaf VAD - try ensuring all non-audio frames are seen by VAD (cleanup documentation here)
            await self._vad_controller.process_frame(frame)
