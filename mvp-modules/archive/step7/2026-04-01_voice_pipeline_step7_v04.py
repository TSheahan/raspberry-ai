"""Step 7 v04 — Wake word + VAD + STT + agentic layer (claude -p).
...
v04 changes from v03:
  - Stop PyAudio stream before teardown. Pipecat's BaseInputTransport.cancel() cancels
    the audio *task* but does not stop the PyAudio *stream*. The stream keeps firing
    callbacks every 20ms throughout CancelFrame propagation and asyncio teardown. On Pi 4
    with USB audio, this race causes kernel-level hangs. We patch the input transport's
    cancel() to stop the stream immediately, then allow a brief drain before continuing.

Provenance: builds on voice_pipeline_step7_v03.py.
"""
...
def patch_transport_cancel(input_transport):
    ...
    Pipecat's default cancel path:
      1. BaseInputTransport.cancel() — cancels the audio asyncio task
      2. CancelFrame propagates through pipeline
      3. LocalAudioInputTransport.cleanup() — finally stops PyAudio stream

    Between steps 1 and 3, PyAudio's callback thread keeps firing every 20ms,
    pushing frames into an event loop that's tearing down. On Pi 4 + USB audio,
    this causes kernel-level USB hangs → reboot.

    This patch stops the stream at step 1, so no new frames enter the pipeline
    during teardown.
    """
    original_cancel = input_transport.cancel

    async def cancel_with_stream_stop(frame):
        # Stop frame production first
        if input_transport._in_stream:
            print("  (stopping PyAudio stream before teardown)")
            input_transport._in_stream.stop_stream()
        # Brief drain: let any frames already in queues propagate
        await asyncio.sleep(0.1)
        # Proceed with normal cancel (cancels audio task, etc.)
...
class OpenWakeWordProcessor(FrameProcessor):
    ...
    def _reset_model_full(self):
        ...
        model.reset() only clears the prediction_buffer deque. The preprocessor
        retains raw_data_buffer, melspectrogram_buffer, feature_buffer, and
        accumulated_samples from before gating. Stale features in these buffers
        cause false wake detections when gating lifts.
        """
        self.model.reset()

        # Preprocessor audio/feature buffers
        ...
        # Clear local chunk buffer
        self._chunks = []

        # Push debounce forward so we don't re-trigger immediately
    ...
    async def process_frame(self, frame: Frame, direction):
        ...
        if isinstance(frame, AudioRawFrame):
            ...
            # Detect gated → ungated transition: reset stale OWW state
...
async def main():
    ...
    # Get the input transport and patch its cancel to stop PyAudio stream first
    ...
    print("Step 7 v04 -- Wake word + VAD + STT + Claude")
